...
class DroplessMoESwiGLUFFN(nn.Module):
    ...
    Position-level routing version:
      - one router decision is made per board position / batch item, not per
        square token;
      - all H*W squares in the same position use the same top-2 experts;
      - expert matmuls are still packed by expert id and executed with
        torch.nn.functional.grouped_mm.

    This keeps channel semantics consistent across the board, while reducing
    routing/top-k/sort work from B*H*W decisions to B decisions.
    """
    ...
    def __init__(self, in_features, hidden_features, num_experts=8):
        ...
        self.register_buffer("_flat_position_cache", torch.empty(0, dtype=torch.long), persistent=False)
        ...
        self.last_expert_token_counts = None
        self.last_expert_load_entropy = None
        self.last_expert_max_load_ratio = None
        self.last_router_entropy = None
        self.last_top1_gate = None
        self.last_top2_gate = None
        self.last_gate_margin = None

        self.reset_parameters()

    def reset_parameters(self):
        # Router: keep it small. Zero init causes top-k tie breaking to select
        # fixed experts at startup, so use small random values instead.
        nn.init.normal_(self.router.weight, mean=0.0, std=0.005)

        # Expert FFN init tuned for SwiGLU residual branches. This replaces the
        # old std=0.02 for both matrices, which made the MoE branch too weak.
    ...
    def _get_flat_position(self, batch_size, device):
        needed = batch_size * self.top_k
        cache = self._flat_position_cache
        if cache.device != device or cache.numel() != needed:
            cache = torch.arange(batch_size, device=device).repeat_interleave(self.top_k)
            self._flat_position_cache = cache
        return cache
    ...
    def forward(self, x):
        ...
        assignments = batch_size * self.top_k

        # Routing is position-level: pool over board squares and route each
        # batch item once. Router is kept in FP32 even under BF16 autocast.
        with torch.autocast(device_type="cuda", enabled=False):
            route_input = x.float().mean(dim=(2, 3))
        ...
        # Expert computation works on [position, square, channel]. Compared to
        # token-level routing, the expensive sort is now over B*top_k assignments
        # instead of B*H*W*top_k token assignments.
        x_tokens = x.permute(0, 2, 3, 1).contiguous().view(batch_size, spatial, channels)
        ...
        flat_position = self._get_flat_position(batch_size, x.device)

        order = torch.argsort(flat_expert)
        expert_sorted = flat_expert.index_select(0, order)
        position_sorted = flat_position.index_select(0, order)
        gate_sorted = flat_gate.index_select(0, order)

        # [B*K, H*W, C], sorted by expert. Flatten only after sorting so each
        # selected position remains a contiguous block for grouped_mm.
        x_sorted = x_tokens.index_select(0, position_sorted)
        x_sorted_flat = x_sorted.reshape(assignments * spatial, channels)

        counts_positions = torch.bincount(expert_sorted, minlength=self.num_experts)
        counts_rows = counts_positions * spatial
        offsets = torch.cumsum(counts_rows, dim=0).to(torch.int32)

        # Optional logging stats for expert-load collapse checks. These are
        # disabled by default to avoid extra reductions every forward pass.
        # counts_f = counts_positions.to(torch.float32)
        # load_prob = counts_f / counts_f.sum().clamp_min(1.0)
        # self.last_expert_counts = counts_positions.detach()
        # self.last_expert_token_counts = counts_rows.detach()
        ...
        # if self.training:
        #     router_entropy = -(probs * probs.clamp_min(1e-12).log()).sum(dim=-1).mean()
        #     self.last_router_entropy = router_entropy.detach()
        #     self.last_top1_gate = topk_gate[:, 0].mean().detach()
        #     self.last_top2_gate = topk_gate[:, 1].mean().detach()
        #     self.last_gate_margin = (topk_gate[:, 0] - topk_gate[:, 1]).mean().detach()

        if self.training:
            # Because every selected position contributes the same number of
            # squares, position counts and token-row counts give the same load
            # distribution after normalization. Use position counts to avoid
            # unnecessary large reductions.
            tokens_per_expert = counts_positions.to(probs.dtype) / counts_positions.sum().to(probs.dtype)
        ...
        padding = self._get_zero_row(x_sorted_flat.shape[-1], x_sorted_flat.dtype, x_sorted_flat.device, "_padding_cache")
        x_grouped = torch.cat((x_sorted_flat, padding), dim=0)
        h = F.grouped_mm(x_grouped, w1, offs=offsets)[: x_sorted_flat.shape[0]]
        h = h.view(assignments, spatial, 2 * self.hidden_features)
        ...
        # Keep each selected position as one block while using grouped_mm over
        # flattened square rows.
        h_flat = h.reshape(assignments * spatial, self.hidden_features)
        hidden_padding = self._get_zero_row(h_flat.shape[-1], h_flat.dtype, h_flat.device, "_hidden_padding_cache")
        h_grouped = torch.cat((h_flat, hidden_padding), dim=0)
        y_sorted = F.grouped_mm(h_grouped, w2, offs=offsets)[: h_flat.shape[0]]
        y_sorted = y_sorted.view(assignments, spatial, channels)
        y_sorted = y_sorted * gate_sorted.to(dtype=y_sorted.dtype).view(assignments, 1, 1)

        # Undo the expert-id sort back to [B, top_k, H*W, C], then reduce top-k.
        # The index_copy operates on whole position blocks instead of individual
        # token rows, which is cheaper than the token-level implementation.
        y_assign = torch.empty(
            assignments,
            spatial,
            channels,
        ...
        y_assign.index_copy_(0, order, y_sorted.to(dtype=y_assign.dtype))
        y = y_assign.view(batch_size, self.top_k, spatial, channels).sum(dim=1)
...
class AttentionBlock(nn.Module):
    def __init__(self, d_model, nhead, activation, moe_num_experts=4):
