...
public:
	UCTSearcher(UCTSearcherGroup* grp, const int thread_id, const int policy_value_batch_maxsize) :
		...
		y1 = new DType[MAX_SLIM_MOVE_LABEL_NUM * policy_value_batch_maxsize];
		...
		checkCudaErrors(cudaHostAlloc((void**)&y1, (size_t)MAX_SLIM_MOVE_LABEL_NUM * policy_value_batch_maxsize * sizeof(DType), cudaHostAllocPortable));
...
void UCTSearcher::EvalNode() {
	...
	const DType(*logits)[MAX_SLIM_MOVE_LABEL_NUM] = reinterpret_cast<DType(*)[MAX_SLIM_MOVE_LABEL_NUM]>(y1);
	...
	for (int i = 0; i < policy_value_batch_size; i++, logits++, value++) {
		...
		for (int j = 0; j < child_num; j++) {
			...
			const int move_label = make_slim_move_label((u16)move.proFromAndTo(), color);
