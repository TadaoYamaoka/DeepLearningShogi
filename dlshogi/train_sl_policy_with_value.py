...
parser = argparse.ArgumentParser(description='Train sl policy network with value')
...
parser.add_argument('--model', type=str, default='model_sl_val', help='model file name')
parser.add_argument('--state', type=str, default='state_sl_val', help='state file name')
...
def mini_batch(hcpevec, volatile=False):
    ...
    cppshogi.hcpe_decode_with_move_result(hcpevec, features1, features2, move, result)
...
for e in range(args.epoch):
    ...
    for i in range(0, len(train_data) - args.batchsize, args.batchsize):
        x1, x2, t1, t2 = mini_batch(train_data[i:i+args.batchsize])
        y1, y2 = model(x1, x2)
        ...
        if optimizer.t % eval_interval == 0:
            x1, x2, t1, t2 = mini_batch(np.random.choice(test_data, 640), volatile=True)
    ...
    for i in range(0, len(test_data) - args.batchsize, args.batchsize):
        x1, x2, t1, t2 = mini_batch(test_data[i:i+args.batchsize], volatile=True)
        y1, y2 = model(x1, x2, test=True)
        itr_test += 1
