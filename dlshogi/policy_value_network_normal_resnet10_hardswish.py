...
class PolicyValueNetwork(nn.Module):
    ...
    def __call__(self, x1, x2):
        ...
        u1_2 = self.l1_2(x2)
        u1 = self.hardswish(self.norm1(u1_1_1 + u1_1_2 + u1_2))
        # Residual block
        h2 = self.hardswish(self.norm2(self.l2(u1)))
        h3 = self.norm3(self.l3(h2))
        u3 = self.hardswish(h3 + u1)
        # Residual block
        h4 = self.hardswish(self.norm4(self.l4(u3)))
        h5 = self.norm5(self.l5(h4))
        u5 = self.hardswish(h5 + u3)
        # Residual block
        h6 = self.hardswish(self.norm6(self.l6(u5)))
        h7 = self.norm7(self.l7(h6))
        u7 = self.hardswish(h7 + u5)
        # Residual block
        h8 = self.hardswish(self.norm8(self.l8(u7)))
        h9 = self.norm9(self.l9(h8))
        u9 = self.hardswish(h9 + u7)
        # Residual block
        h10 = self.hardswish(self.norm10(self.l10(u9)))
        h11 = self.norm11(self.l11(h10))
        u11 = self.hardswish(h11 + u9)
        # Residual block
        h12 = self.hardswish(self.norm12(self.l12(u11)))
        h13 = self.norm13(self.l13(h12))
        u13 = self.hardswish(h13 + u11)
        # Residual block
        h14 = self.hardswish(self.norm14(self.l14(u13)))
        h15 = self.norm15(self.l15(h14))
        u15 = self.hardswish(h15 + u13)
        # Residual block
        h16 = self.hardswish(self.norm16(self.l16(u15)))
        h17 = self.norm17(self.l17(h16))
        u17 = self.hardswish(h17 + u15)
        # Residual block
        h18 = self.hardswish(self.norm18(self.l18(u17)))
        h19 = self.norm19(self.l19(h18))
        u19 = self.hardswish(h19 + u17)
        # Residual block
        h20 = self.hardswish(self.norm20(self.l20(u19)))
        h21 = self.norm21(self.l21(h20))
        u21 = self.hardswish(h21 + u19)
        # policy network
        h22 = self.l22(u21)
        h22_1 = self.l22_2(h22.view(-1, 9*9*MAX_MOVE_LABEL_NUM))
        # value network
        h22_v = self.hardswish(self.norm22_v(self.l22_v(u21)))
        h23_v = self.hardswish(self.l23_v(h22_v.view(-1, 9*9*MAX_MOVE_LABEL_NUM)))
        return h22_1, self.l24_v(h23_v)

    def set_hardswish(self, export_friendly=True):
        self.hardswish = Hardswish() if export_friendly else nn.Hardswish()
