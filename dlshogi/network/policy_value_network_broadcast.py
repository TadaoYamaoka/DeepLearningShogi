...
class BroadcastResBlock(nn.Module):
    def __init__(self, channels, activation):
        ...
        self.broadcast = nn.Conv2d(1, 9 * 9, kernel_size=9, bias=False)
        self.bn2 = nn.BatchNorm2d(1)
        self.conv2 = nn.Conv2d(1, channels, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        out = self.bn2(out.reshape(-1, 1, 9, 9))
        out = self.act(out)
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        # Resnet blocks
        self.blocks = nn.Sequential(*[BroadcastResBlock(channels, activation) if i % 7 == 0 else ResNetBlock(channels, activation) for i in range(blocks)])
