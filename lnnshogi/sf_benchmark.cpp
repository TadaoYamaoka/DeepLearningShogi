...
std::vector<std::string> setup_bench(const std::string& currentFen, std::istream& is) {
    ...
    list.emplace_back("usinewgame");

    for (const std::string& sfen : fens)
        if (sfen.find("setoption") != std::string::npos)
            list.emplace_back(sfen);
        else
        {
            list.emplace_back("position fen " + sfen);
...
BenchmarkSetup setup_benchmark(std::istream& is) {
    ...
    for (const auto& game : BenchmarkPositions)
    {
        setup.commands.emplace_back("usinewgame");
        int ply = 1;
        for (const std::string& sfen : game)
        {
            setup.commands.emplace_back("position fen " + sfen);
