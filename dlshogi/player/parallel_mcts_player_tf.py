...
def sigmoid(x):
    ...
    if x >= sigmoid_range:
        return 1.0 - 1e-15
    return np.array(1.0 / (1.0 + np.exp(-x)), dtype=np.float32)
...
class ParallelMCTSPlayer(BasePlayer):
    ...
    def eval_node(self):
        with self.graph.as_default():
            ...
            while True:
                ...
                y1 = np.array(predictions['policy_head_notop'].data, dtype=np.float32)
                y2 = np.array(predictions['value_head'].data, dtype=np.float32)
                logits_batch = y1
                values_batch = sigmoid(y2)
    ...
    def isready(self):
        ...
        if self.model is None:
            ...
            # print(self.model.get_layer(layer_name).output)
