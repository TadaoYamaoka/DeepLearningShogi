...
class AttentionPolicy(nn.Module):
    def __init__(self, d_model):
        ...
        self.qkv = nn.Conv2d(d_model, 2 * d_model, kernel_size=1)
        self.rel_bias = nn.Parameter(torch.zeros([81, 81]))


    def forward(self, x):
        qkv = self.qkv(x)
        q, k, v = qkv.split((self.d_model // 2, self.d_model // 2, self.d_model), dim=1)


        q = q.view(-1, self.d_model // 2, 81).transpose(1, 2)
        k = k.view(-1, self.d_model // 2, 81)
        v = v.view(-1, self.d_model, 81)
        ...
        out = torch.matmul(v, attention.transpose(1, 2))
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        # policy network
        self.policy_attn = AttentionPolicy(channels)
    ...
    def forward(self, x1, x2):
        ...
        # policy network
        h_policy = self.policy_attn(h)
        h_policy = self.policy_conv(h_policy)
