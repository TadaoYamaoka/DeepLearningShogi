	...
	do {
		...
		for (int i = 0; i < policy_value_batch_maxsize; i++) {
			...
			if (leaf_search_depth == 0) {
				th = pos_root->searcher()->threads.main();
				s = pos_root->searcher()->thisptr;
			}
			else {
				th = searchers[thread_id].threads.main();
				s = searchers[thread_id].thisptr;
			}
			Position pos(*pos_root, th);
			pos.setSearcher(s);
