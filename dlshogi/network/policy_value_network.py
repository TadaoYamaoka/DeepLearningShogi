...
def policy_value_network(network, add_sigmoid=False):
    ...
    elif network[:9] == 'broadcast':
        from dlshogi.network.policy_value_network_broadcast import PolicyValueNetwork
    else:
    ...
    elif network[:6] == 'resnet' or network[:5] == 'senet' or network[:9] == 'broadcast':
        m = re.match('^(resnet|senet|broadcast)(\d+)(x\d+){0,1}(_fcl\d+){0,1}(_reduction\d+){0,1}(_.+){0,1}$', network)
