...
def policy_value_network(network, add_sigmoid=False):
    ...
    elif network[-6:] == '_fixup':
        from dlshogi.network.policy_value_network_resnet_fixup import PolicyValueNetwork
    elif network[-7:] == '_fixup2':
        from dlshogi.network.policy_value_network_resnet_fixup2 import PolicyValueNetwork
    elif network[-8:] == '_nofixup':
        from dlshogi.network.policy_value_network_resnet_nofixup import PolicyValueNetwork
    ...
    if network in [ 'wideresnet10', 'resnet10_swish' ]:
        ...
        m = re.match('^(resnet|senet)(\d+)(x\d+){0,1}(_fcl\d+){0,1}(_reduction\d+){0,1}(_.+?){0,1}(_fixup|_fixup2|_nofixup|)$', network)
