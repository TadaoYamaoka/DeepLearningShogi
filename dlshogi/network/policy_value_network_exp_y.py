...
class InceptionBlock(nn.Module):
    def __init__(self, channels, activation):
        ...
        self.conv2 = nn.Conv2d(channels * 3, channels, kernel_size=1, bias=False)
        self.bn2 = nn.BatchNorm2d(channels)
        self.act = activation
    ...
    def forward(self, x):
        out = torch.cat((
            self.bn1_1(self.conv1_1(x)).expand(-1, -1, 9, -1),
            self.bn1_2(self.conv1_2(x)).expand(-1, -1, -1, 9),
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        self.act = activation
        self.pos = nn.Parameter(torch.zeros([1, channels, 9, 9]))
    ...
    def forward(self, x1, x2):
        ...
        u1 = self.act(u1_1_1 + u1_1_2 + u1_2) + self.pos
