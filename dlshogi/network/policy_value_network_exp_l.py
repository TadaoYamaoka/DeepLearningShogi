...
class MHSA(nn.Module):
    def __init__(self, d_model, nhead):
        ...
        self.o = nn.Conv2d(d_model, d_model, kernel_size=1)
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        # policy network
        self.policy_mhsa = MHSA(channels, 4)
        self.policy_block = ResNetBlock(channels, activation)
    ...
    def forward(self, x1, x2):
        ...
        # policy network
        h_policy = self.policy_mhsa(h)
        h_policy = self.policy_block(h_policy)
