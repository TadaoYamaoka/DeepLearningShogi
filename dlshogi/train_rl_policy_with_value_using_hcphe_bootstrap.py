...
from dlshogi.policy_value_history_network import *
...
model = PolicyValueHistoryNetwork()
model.to_gpu()
...
train_data = np.fromfile(args.train_data, dtype=HuffmanCodedPosWithHistoryAndEval)
test_data = np.fromfile(args.test_data, dtype=HuffmanCodedPosWithHistoryAndEval)
...
def mini_batch(hcpevec):
    features1 = np.empty((len(hcpevec), FEATURES1_HIST_NUM, 9, 9), dtype=np.float32)
    features2 = np.empty((len(hcpevec), FEATURES2_HIST_NUM, 9, 9), dtype=np.float32)
    ...
    cppshogi.hcphe_decode_with_value(hcpevec, features1, features2, move, result, value)
