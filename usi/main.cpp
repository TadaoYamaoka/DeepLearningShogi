...
void MySearcher::makeBook(std::istringstream& ssCmd, const std::string& posCmd) {
	...
	init_usi_book_engine(options["USI_Book_Engine"], options["USI_Book_Engine_Options"], options["USI_Book_Engine_Nodes"], options["USI_Book_Engine_Prob"] / 1000.0, options["USI_Book_Engine_Nodes_Own"], options["USI_Book_Engine_Prob_Own"] / 1000.0, options["USI_Book_Engine_Num"], options["USI_Book_Engine_Random_Nodes"]);
...
void MySearcher::makeAllMinMaxBook(std::istringstream& ssCmd, const std::string& posCmd) {
	...
	// 定跡をmin-max探索
	std::map<Key, std::vector<BookEntry> > outMap;
	make_all_minmax_book(pos, outMap, make_book_color, threads, bookMapBest, chunk_num, chunk_index);
...
void MySearcher::makePolicyBookBfsRA(std::istringstream& ssCmd, const std::string& posCmd) {
	HuffmanCodedPos::init();
	std::string bookFileName;
	std::string outFileName;
	int threads = 1;
	...
	// 定跡をmin-max探索
	std::map<Key, std::vector<BookEntry> > outMap;
	make_policy_book_bfs_ra(pos, outMap, threads, bookMapBest, 1.0 / temperature);
...
void MySearcher::bookToHcp(std::istringstream& ssCmd, const std::string& posCmd) {
	HuffmanCodedPos::init();
...
void MySearcher::complementBook(std::istringstream& ssCmd, const std::string& posCmd) {
	...
	complement_book(pos, bookFileName, outFileName, book_pos_cmd);
...
// 互角局面集作成
void MySearcher::makeGokakuSfen(std::istringstream& ssCmd, const std::string& posCmd) {
	HuffmanCodedPos::init();
...
// 重要局面集作成
void MySearcher::makeImportantHcp(std::istringstream& ssCmd, const std::string& posCmd) {
	HuffmanCodedPos::init();
	...
	make_important_hcp(pos, posCmd, bookFileName, policyFileName, outFileName, diff);

	std::cout << "done" << std::endl;
}

// 重要局面集作成
void MySearcher::makeImportantSfen(std::istringstream& ssCmd, const std::string& posCmd) {
	HuffmanCodedPos::init();
