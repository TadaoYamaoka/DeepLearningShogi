...
class RFBlock(nn.Module):
    def __init__(self, channels, activation):
        ...
        self.conv1_1 = nn.Conv2d(channels, channels, kernel_size=(9, 1), padding=0, bias=False)
        self.conv1_2 = nn.Conv2d(channels, channels, kernel_size=(1, 9), padding=0, bias=False)
        self.bn1 = nn.BatchNorm2d(channels)
        self.conv2_1 = nn.Conv2d(channels, channels, kernel_size=(9, 1), padding=0, bias=False)
        self.conv2_2 = nn.Conv2d(channels, channels, kernel_size=(1, 9), padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(channels)
        self.act = activation

    def forward(self, x):
        out = self.conv1_1(x) + self.conv1_2(x)
        out = self.bn1(out)
        out = self.act(out)
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        for i in range(blocks):
            if i % 4 == 1:
                _blocks.append(RFBlock(channels, activation))
            else:
    ...
    def forward(self, x1, x2):
        ...
        u1_2 = self.l1_2(x2)
        u1 = self.pos_bias(self.act(self.norm1(u1_1_1 + u1_1_2 + u1_2)))
