...
void MySearcher::doUSICommandLoop(int argc, char* argv[]) {
	...
	do {
		...
		if (token == "quit") {
			...
			SetRandomMove(options["Random_Ply"], options["Random_Temperature"], options["Random_Temperature_Drop"], options["Random_Cutoff"], options["Random_Cutoff_Drop"], options["Random_Ply2"], options["Random_Cutoff2"]);
			SetSuppressCheckRush(options["Suppress_Check_Rush"]);
