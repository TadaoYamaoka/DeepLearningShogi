...
class Hcpe3AuxDataLoader(DataLoader):
    @staticmethod
    def load_files(files):
        sum_sennichite_len = 0
        sum_nyugyoku_len = 0
        for path in files:
            if os.path.exists(path):
                logging.debug(path)
                sum_len, len_, sennichite_len, nyugyoku_len = cppshogi.load_hcpe3(path)
                if len_ == 0:
                    raise RuntimeError('read error {}'.format(path))
                sum_sennichite_len += sennichite_len
                sum_nyugyoku_len += nyugyoku_len
            else:
                logging.debug('{} not found, skipping'.format(path))
        return sum_len, sum_sennichite_len, sum_nyugyoku_len
    ...
    def mini_batch(self, index):
        cppshogi.hcpe3_decode_with_value_and_aux(index, self.features1, self.features2, self.probability, self.result, self.value, self.aux)
