...
class DataLoader:
    ...
    def __init__(self, data, batch_size, device, shuffle=False):
        ...
        self.torch_draw = torch.empty((batch_size, 1), dtype=torch.float32, pin_memory=True)
        ...
        self.draw = self.torch_draw.numpy().reshape(-1)

        self.i = 0
        self.executor = ThreadPoolExecutor(max_workers=1)

    def mini_batch(self, hcpevec):
        cppshogi.hcpe_decode_with_value(hcpevec, self.features1, self.features2, self.move, self.result, self.value, self.draw)
        ...
        if self.device.type == 'cpu':
            ...
            return (self.torch_features1.to(self.device),
                    ...
                    self.torch_draw.to(self.device)
                    )
...
class Hcpe3DataLoader(DataLoader):
    ...
    def mini_batch(self, index):
        cppshogi.hcpe3_decode_with_value(index, self.features1, self.features2, self.probability, self.result, self.value, self.draw)
