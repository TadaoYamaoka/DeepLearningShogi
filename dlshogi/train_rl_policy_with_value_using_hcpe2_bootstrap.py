...
class DataLoader:
    ...
    def mini_batch(self, hcpevec):
        ...
        return (self.torch_features1.to(device, non_blocking=True),
                ...
                self.torch_aux.to(device, non_blocking=True),
...
def hcpe_loader(data, batchsize):
    for x1, x2, t1, t2, t3, z, value in DataLoader(data, batchsize):
        yield x1, x2
...
for e in range(args.epoch):
    ...
    sum_loss4_epoch = 0
    sum_loss_epoch = 0
    for x1, x2, t1, t2, t3, z, value in train_dataloader:
        if args.use_amp:
    ...
    model.eval()
    with torch.no_grad():
        for x1, x2, t1, t2, t3, z, value in test_dataloader:
            y1, y2, y3 = model(x1, x2)
