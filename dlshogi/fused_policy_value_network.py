...
class FusedPolicyValueNetwork(Chain):
    def __init__(self):
        super(FusedPolicyValueNetwork, self).__init__(
            ...
            l14=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1),
            l15=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1, nobias = True),
            l16=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1),
            l17=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1, nobias = True),
            l18=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1),
            l19=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1, nobias = True),
            l20=L.Convolution2D(in_channels = k, out_channels = k, ksize = 3, pad = 1),
    ...
    def __call__(self, x1, x2):
        ...
        h2 = F.dropout(F.relu(self.l2(h1)), ratio=dropout_ratio)
        u3 = self.l3(h2) + u1
        # Residual block
        h3 = F.relu(self.norm3(u3))
        h4 = F.dropout(F.relu(self.l4(h3)), ratio=dropout_ratio)
        u5 = self.l5(h4) + u3
        # Residual block
        h5 = F.relu(self.norm5(u5))
        h6 = F.dropout(F.relu(self.l6(h5)), ratio=dropout_ratio)
        u7 = self.l7(h6) + u5
        # Residual block
        h7 = F.relu(self.norm7(u7))
        h8 = F.dropout(F.relu(self.l8(h7)), ratio=dropout_ratio)
        u9 = self.l9(h8) + u7
        # Residual block
        h9 = F.relu(self.norm9(u9))
        h10 = F.dropout(F.relu(self.l10(h9)), ratio=dropout_ratio)
        u11 = self.l11(h10) + u9
        # Residual block
        h11 = F.relu(self.norm11(u11))
        h12 = F.dropout(F.relu(self.l12(h11)), ratio=dropout_ratio)
        u13 = self.l13(h12) + u11
        # Residual block
        h13 = F.relu(self.norm13(u13))
        h14 = F.dropout(F.relu(self.l14(h13)), ratio=dropout_ratio)
        u15 = self.l15(h14) + u13
        # Residual block
        h15 = F.relu(self.norm15(u15))
        h16 = F.dropout(F.relu(self.l16(h15)), ratio=dropout_ratio)
        u17 = self.l17(h16) + u15
        # Residual block
        h17 = F.relu(self.norm17(u17))
        h18 = F.dropout(F.relu(self.l18(h17)), ratio=dropout_ratio)
        u19 = self.l19(h18) + u17
        # Residual block
        h19 = F.relu(self.norm19(u19))
        h20 = F.dropout(F.relu(self.l20(h19)), ratio=dropout_ratio)
