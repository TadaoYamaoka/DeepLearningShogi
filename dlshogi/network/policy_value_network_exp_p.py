...
class AttentionPolicy(nn.Module):
    def __init__(self, d_model, nhead, activation):
        ...
        torch.nn.init.constant_(self.bn.weight, 0)
    ...
    def forward(self, x):
        ...
        q = q.view(-1, self.nhead, self.depth // 2, 81).transpose(2, 3)
        k = k.view(-1, self.nhead, self.depth // 2, 81)
...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        # policy network
        self.policy_attn = AttentionPolicy(channels, 4, nn.SELU())
