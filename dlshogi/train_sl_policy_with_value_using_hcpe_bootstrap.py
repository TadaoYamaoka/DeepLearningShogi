...
parser = argparse.ArgumentParser(description='Traning SL policy network with value using hcpe')
...
parser.add_argument('--model', type=str, default='model_sl_val_hcpe', help='model file name')
parser.add_argument('--state', type=str, default='state_sl_val_hcpe', help='state file name')
...
for e in range(args.epoch):
    ...
    for i in range(0, len(train_data) - args.batchsize, args.batchsize):
        ...
        if optimizer.t % eval_interval == 0:
            x1, x2, t1, t2, value = mini_batch(np.random.choice(test_data, 640))
    ...
    for i in range(0, len(test_data) - args.batchsize, 640):
        x1, x2, t1, t2, value = mini_batch(test_data[i:i+args.batchsize])
        with chainer.no_backprop_mode():
            with chainer.using_config('train', False):
                y1, y2 = model(x1, x2)
        itr_test += 1
