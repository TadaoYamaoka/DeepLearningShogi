...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256):
        ...
        self.l1_1_3 = nn.Conv2d(in_channels=FEATURES1_NUM, out_channels=channels, kernel_size=(1, 9), padding=0, bias=False)
        self.l1_1_4 = nn.Conv2d(in_channels=FEATURES1_NUM, out_channels=channels, kernel_size=(9, 1), padding=0, bias=False)
        self.l1_2 = nn.Conv2d(in_channels=FEATURES2_NUM, out_channels=channels, kernel_size=1, bias=False) # pieces_in_hand
        self.norm1 = nn.BatchNorm2d(channels)
        self.act = activation
        self.pos_bias = Bias((channels, 9, 9))

        # Resnet blocks
        self.blocks = nn.Sequential(*[ResNetBlock(channels, activation) for _ in range(blocks)])

        # policy network
        self.policy_conv1 = nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=1, bias=False)
        self.mha = nn.MultiheadAttention(32, 8, batch_first=True)
        self.policy_conv2 = nn.Conv2d(in_channels=32, out_channels=MAX_MOVE_LABEL_NUM, kernel_size=1, bias=False)
        self.policy_bias = Bias(9*9*MAX_MOVE_LABEL_NUM)

        # value network
        self.value_conv1 = nn.Conv2d(in_channels=channels, out_channels=32, kernel_size=1, bias=False)
        self.value_norm1 = nn.BatchNorm2d(32)
        self.value_fc1 = nn.Linear(9*9*32, fcl)
    ...
    def forward(self, x1, x2):
        ...
        u1_2 = self.l1_2(x2)
        u1 = self.pos_bias(self.act(self.norm1(u1_1_1 + u1_1_2 + u1_1_3 + u1_1_4 + u1_2)))

        # resnet blocks
        h = self.blocks(u1)

        # policy network
        h_policy = self.policy_conv1(h)
        h_policy = h_policy.view(-1, 32, 81).transpose(1, 2)
        h_policy = self.mha(h_policy, h_policy, h_policy)[0].transpose(1, 2).view(-1, 32, 9, 9)
        h_policy = self.policy_conv2(h_policy)
