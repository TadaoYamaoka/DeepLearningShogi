...
const int slim_move_label_map[][SquareNum] = {
...
int make_slim_move_label(const u16 move16, const Color color) {
	...
	if (from_sq < SquareNum) {
		...
		assert(move_direction < MOVE_DIRECTION_NUM);
		return slim_move_label_map[move_direction][to_sq];
	}
	// 持ち駒の場合
	else {
		// 白の場合、盤面を180度回転
		...
		const int hand_piece = from_sq - (int)SquareNum;
		const int move_direction_label = MOVE_DIRECTION_NUM + hand_piece;
		assert(move_direction_label < MAX_MOVE_LABEL_NUM);
		return slim_move_label_map[move_direction_label][to_sq];
	}
}
