...
void iter_diff_eval(Position& root_pos, std::unordered_map<Key, std::vector<BookEntry>>& outMap, std::unordered_map<Key, std::vector<BookEntry>>& outEngineMap, const std::unordered_map<Key, std::vector<BookEntry>>& policyMap, const std::string& engine_path, const std::string& engine_options, const int nodes, const int engine_num, LimitsType& limits, const Score diff, const Score threashold, const Score opp_threashold, const std::string& outEngineFileName, const std::string& outFileName, const std::string& book_pos_cmd, const Key& book_starting_pos_key);
