...
from dlshogi.data_loader_lite import Hcpe3DataLoader
from dlshogi.data_loader_lite import DataLoader
...
def main(*argv):
    ...
    logging.info('dims {}'.format(args.dims))
    ...
    def hcpe_loader(data, batchsize):
        for x1, x2, result, value in Hcpe3DataLoader(data, batchsize, device):
            yield { 'x1':x1, 'x2':x2 }
    ...
    def test(model):
        ...
        return (sum_test_loss1 / steps,
                ...
                sum_test_loss / steps,
                sum_test_accuracy / steps,
                sum_test_entropy / steps)
    ...
    for e in range(args.epoch):
        ...
        for x1, x2, result, value in train_dataloader:
            ...
            if t % eval_interval == 0:
                ...
                x1, x2, result, value = test_dataloader.sample()
                with torch.no_grad():
                    y = model(x1, x2)
                    ...
                    logging.info('epoch = {}, steps = {}, train loss = {:.07f}, {:.07f}, {:.07f}, test loss = {:.07f}, {:.07f}, {:.07f}, test accuracy = {:.07f}'.format(
                        epoch, t,
                        sum_loss1 / steps, sum_loss2 / steps, sum_loss / steps,
                        loss1.item(), loss2.item(), loss.item(),
                        binary_accuracy(y, result)))
        ...
        test_loss1, test_loss2, test_loss, test_accuracy, test_entropy = test(model)

        logging.info('epoch = {}, steps = {}, train loss avr = {:.07f}, {:.07f}, {:.07f}, test loss = {:.07f}, {:.07f}, {:.07f}, test accuracy = {:.07f}, test entropy = {:.07f}'.format(
            epoch, t,
            sum_loss1_epoch / steps_epoch, sum_loss2_epoch / steps_epoch, sum_loss_epoch / steps_epoch,
            test_loss1, test_loss2, test_loss,
            test_accuracy,
            test_entropy))
    ...
    if args.model:
        if args.use_swa and epoch >= args.swa_start_epoch:
            ...
            test_loss1, test_loss2, test_loss, test_accuracy, test_entropy = test(swa_model)
